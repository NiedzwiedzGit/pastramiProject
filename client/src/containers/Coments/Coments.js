...
const Coments = React.memo(props => {
    ...
    useEffect(() => {
        async function anyNameFunction() {
            dbF.ref(`coment`).on("value", snapshot => {
                let comentVarArr = [];
                ...
                setComentVar(comentVarArr.reverse());
            })
        }
        anyNameFunction();
    ...
    let comentsHandler = () => {
        let date = `${new Date().getFullYear()}-${(new Date().getMonth() < 10 ? '0' : '') + (new Date().getMonth() + 1)}-${(new Date().getDate() < 10 ? '0' : '') + new Date().getDate()}  ${new Date().getHours()}:${(new Date().getMinutes() < 10 ? '0' : '') + new Date().getMinutes()}`;
    ...
    let onLoadComent = () => {
        ...
        />;
        if (comentVar !== null) {
            if (comentVar.length !== 0) {
                let sortedArr = comentVar.sort(function (a, b) { return a - b });
    ...
    return (
        <div className={classes.Coments}  {...swipe.handlers(props.history, '/')}>
            ...
            <div className={[classes.Draggable, classes[props.showContentEditWraper ? 'DraggableActive' : null]].join(' ')}>
                ...
                >
                    {props.showContentEditWraper ? <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-chevron-double-up" viewBox="0 0 16 16">
